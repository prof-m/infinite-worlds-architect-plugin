 ...
 */
function normalizeQueryArgs(argsOrExtractionDir, category, turns) {
  if (
    argsOrExtractionDir !== null &&
    typeof argsOrExtractionDir === 'object' &&
    !Array.isArray(argsOrExtractionDir)
  ) {
    return {
      extractionDir: argsOrExtractionDir.extraction_dir,
      category: argsOrExtractionDir.category,
      turns: argsOrExtractionDir.turns,
    };
  }

  return {
    extractionDir: argsOrExtractionDir,
    category,
    turns,
  };
}

export async function queryStoryData(argsOrExtractionDir, categoryArg, turnsArg) {
  const {
    extractionDir,
    category,
    turns,
  } = normalizeQueryArgs(argsOrExtractionDir, categoryArg, turnsArg);
  ...
  try {
    ...
    switch (category) {
      case 'character_index': {
        const characterIndexPath = path.join(extractionDir, 'character_index.json');
        try {
          const characterIndex = await readJsonFile(characterIndexPath);
          return {
            success: true,
            category,
            data: characterIndex,
          ...
          if (err.code === 'ENOENT') {
            return {
              success: false,
              category,
              error: 'No character index found in this extraction. Re-run extract_story_data on a world file that includes character data.',
            };
          }
          throw err;
        }
      ...
      case 'turn_detail': {
        ...
        // Initialize file cache only when querying 5+ turns
        const useCache = resolvedTurns.length >= 5;
        const fileCache = useCache ? {} : null;
        ...
        for (const turnNum of resolvedTurns) {
          ...
          try {
            ...
            if (useCache && fileCache[resolvedPath]) {
              sourceContent = fileCache[resolvedPath];
            } else {
              sourceContent = await fs.promises.readFile(resolvedPath, 'utf8');
              // Store in cache if enabled
              if (useCache) {
                fileCache[resolvedPath] = sourceContent;
              }
            }
          } catch (err) {
