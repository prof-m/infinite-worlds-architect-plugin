 ...
 * @param {Object} characterIndex - Optional character index data
 * @returns {Promise<{filesWritten: string[], warnings: string[]}>}
 */
export async function writeOutputFiles(extractionDir, parsedHeader, parsedTurns, snapshots, manifest, characterIndex = null) {
  const filesWritten = [];
  const warnings = [];
  ...
  try {
    ...
    // Build character_index.json only if characters were provided
    if (characterIndex) {
      const characterIndexPath = path.join(extractionDir, 'character_index.json');
      await writeJsonFile(characterIndexPath, characterIndex);
      filesWritten.push('character_index.json');
    }
