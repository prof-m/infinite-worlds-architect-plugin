...
function validateEvidence(evidence) {
    ...
    // Story-citation prefixes (no minimum char requirement beyond non-empty)
    const storyCitationPrefixes = [
        'From Turn #',
        'From Story Metadata',
        'From Turn Detail',
    ];
    for (const prefix of storyCitationPrefixes) {
        if (trimmed.startsWith(prefix)) return trimmed;
    }

    // Special case: "From Turn #N Tracked Item" — already covered by "From Turn #" prefix above

    // Prefix-tagged with minimum 10 non-whitespace chars after prefix
    const prefixedKinds = ['USER_DIRECTED:', 'CARRY_FORWARD:', 'NO_STORY_EVIDENCE:'];
    for (const prefix of prefixedKinds) {
        if (trimmed.startsWith(prefix)) {
            const rest = trimmed.slice(prefix.length);
            const nonWs = rest.replace(/\s/g, '');
            if (nonWs.length < 10) {
                throw new Error(EVIDENCE_ERROR_MESSAGE);
            }
            return trimmed;
...
async function parseDraft(draftPath) {
    ...
    for (const section of sections) {
        ...
        // Strip evidence comment if present as lines[1]
        let contentStartIndex = 1;
        if (lines.length > 1) {
            const evidenceMatch = lines[1].match(/^\s*<!-- evidence: (.*) -->\s*$/);
            if (evidenceMatch) {
...
export async function read_draft_section(args) {
    ...
    for (const section of sections) {
        ...
        if (header === targetHeader) {
            // Strip evidence comment if present
            let startIdx = 1;
            if (lines.length > 1 && lines[1].match(/^\s*<!-- evidence: .* -->\s*$/)) {
                startIdx = 2;
            }
