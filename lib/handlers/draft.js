...
export async function update_draft_section(args) {
    ...
    if (grounded) {
        // Validate evidence and build the comment only for story_grounded drafts
    ...
    // Strip any injected evidence comments from the start of newContent.
    // Only applies when grounded — prevents a caller from embedding a fake evidence comment
    // that would override the validated one when parseDraft reads the section back.
    let newContent = args.newContent;
    if (grounded) {
        const contentLines = newContent.split('\n');
        let idx = 0;
        // Skip leading blank lines, then remove consecutive leading evidence comment lines.
        while (idx < contentLines.length && contentLines[idx].trim() === '') idx++;
        const firstContentIdx = idx;
        while (idx < contentLines.length && /^\s*<!-- evidence:.*-->\s*$/.test(contentLines[idx])) {
            idx++;
        }
        // Only rewrite newContent if at least one evidence line was found and removed.
        if (idx > firstContentIdx) {
            newContent = contentLines.slice(idx).join('\n');
        }
    }
    ...
    for (let i = 1; i < sections.length; i++) {
        ...
        if (header.toLowerCase() === sectionName) {
            if (evidenceComment) {
                ...
                // Unmarked draft: write section without evidence comment
                result += '# ' + header + '\n' + newContent + '\n\n';
            }
            found = true;
        } else {
