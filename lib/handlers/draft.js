        ...
        if (header === 'title') parsed.title = content;
            ...
            for (const sf of sfEntries) {
                ...
                if (header === 'possible characters') {
                    ...
                    if ('Can Trigger More Than Once' in itemFields && itemFields['Can Trigger More Than Once'].trim().toLowerCase() === 'true') {
                        itemObj.canTriggerMoreThanOnce = true;
                    }
                    if ('Prerequisites' in itemFields) {
                        const prereqs = itemFields['Prerequisites'].split(',').map(s => s.trim()).filter(Boolean);
                        if (prereqs.length) itemObj.prerequisites = prereqs;
                    }
                    if ('Blockers' in itemFields) {
                        const blockerList = itemFields['Blockers'].split(',').map(s => s.trim()).filter(Boolean);
                        if (blockerList.length) itemObj.blockers = blockerList;
                    }
                    parsedItems.push(itemObj);
                }
            }
