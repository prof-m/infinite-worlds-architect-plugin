 ...
 * Preserves the existing format and trailing blank lines exactly.
 * If the field is not found, it is inserted at the end of the body using the body's existing format.
...
function patchFieldInBody(body, fieldName, newValue) {
    ...
    // --- Try H3 format: ### fieldName ---
    let inFence = false;
    let fenceChar = '';
    let h3MatchLine = -1;  // index of the ### fieldName line
    let nextH3Line = -1;   // index of the NEXT ### line after the match
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (!inFence) {
            ...
            const cp = new RegExp(`^[${fenceChar === '`' ? '`' : '~'}]{3,}\\s*$`);
            if (cp.test(trimmed)) inFence = false;
        }
    ...
    if (h3MatchLine !== -1) {
        ...
        // Count trailing blanks in the replaced region to preserve spacing
        ...
        const before = lines.slice(0, h3MatchLine + 1); // includes the ### line
        return {
    ...
    // --- Try Key:Value format: fieldName: value ---
    const escapedName = fieldName.trim().replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    const kvPattern = new RegExp(`^(${escapedName})(:\\s*)(.*)$`, 'i');
    for (let i = 0; i < lines.length; i++) {
        const m = lines[i].match(kvPattern);
        if (m) {
            const newLines = lines.slice();
            newLines[i] = `${m[1]}${m[2]}${newValue}`;
            return { result: newLines.join('\n'), inserted: false };
        }
    }

    // --- Not found: insert at end using detected format ---
    // Use H3 format if the body already has H3 headers OR if body is empty
    // (H3 is the recommended format per draft_schema.md). Fall back to Key:Value only
    // when the body has Key:Value pairs but no H3 headers.
    const hasH3 = lines.some(l => /^###\s+/.test(l));
    const bodyIsEmpty = lines.every(l => l.trim() === '');
    const toInsert = (hasH3 || bodyIsEmpty)
        ? `### ${fieldName.trim()}\n${newValue}`
...
export async function update_draft_field(args) {
    ...
    return successResponse(
        `Successfully patched field '${args.fieldName}' in sub-field '${args.subField}' ` +
