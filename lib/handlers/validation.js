...
/**
 * Validate a world JSON file against the Infinite Worlds schema.
 * @async
 * @param {Object} args - Function arguments
 * @param {string} args.path - Absolute path to the world JSON file to validate
 * @returns {Promise<{content: Array<{type: string, text: string}>}>} MCP response with validation report (errors, warnings, info)
 */
...
/**
 * Audit a world JSON file for token efficiency, instruction density, and design quality.
 * Returns a comprehensive audit report with token cost estimates, instruction density analysis, and optimization suggestions.
 * @async
 * @param {Object} args - Function arguments
 * @param {string} args.path - Absolute path to the world JSON file to audit
 * @returns {Promise<{content: Array<{type: string, text: string}>}>} MCP response with audit report and optimization suggestions
 */
export async function audit_world(args) {
    ...
    for (const item of trackedItems) {
        ...
        if (!referenced) {
            if (alwaysOnText.includes(itemName)) {
                referenced = true;
            } else {
                // Also check instruction blocks and lore entries
    ...
    // triggerOnEvent platform limit
    let triggerOnEventCount = 0;
    for (const t of triggers) {
        for (const cond of (t.triggerConditions || [])) {
            if (cond.type === "triggerOnEvent") triggerOnEventCount++;
        }
    }
    if (triggerOnEventCount > 10) {
        triggerReport += `**WARNING: ${triggerOnEventCount} triggerOnEvent conditions found (platform limit is 10).**\n`;
        recommendations.push(`${triggerOnEventCount} triggerOnEvent conditions found across all triggers. The platform limit is 10 — exceeding this may cause unreliable trigger behaviour.`);
    }
