...
/**
 * Read a world JSON file and return its NPCs as a character list in the format
 * expected by extract_story_data's character_list parameter.
 * @async
 * @param {Object} args - Function arguments
 * @param {string} args.path - Absolute path to the world JSON file
 * @returns {Promise<{content: Array<{type: string, text: string}>}>} MCP response with character_list
 */
export async function getCharacterList(args) {
    ...
    try {
        ...
        throw new Error(`Could not access file at ${worldPath}: ${e.message}`);
    }
    if (!stat.isFile()) {
        throw new Error(`Path is not a file: ${worldPath}`);
    }

    const world = await readWorld(worldPath);
    if (!world) throw new Error(`Could not parse world JSON at ${worldPath}`);

    const npcs = world.NPCs;
    if (!npcs || !Array.isArray(npcs) || npcs.length === 0) {
        return successResponse(JSON.stringify({ character_list: [] }));
    }

    const character_list = npcs.map(npc => {
        const name = npc.name || '';
        const rawNames = Array.isArray(npc.names) ? npc.names : [];
...
/**
 * Locate a file or directory and return its absolute path for user confirmation.
 * @async
 * @param {Object} args - Function arguments
 * @param {string} args.inputPath - The name or partial path provided by the user
 * @param {string} args.type - Whether to look for a file or a directory. One of: "file", "directory"
 * @returns {Promise<{content: Array<{type: string, text: string}>}>} MCP response with resolved path or not found message
 */
...
/**
 * Initialize a new world JSON file with safe, token-efficient defaults.
 * @async
 * @param {Object} args - Function arguments
 * @param {string} args.path - Absolute path where the world JSON file should be saved
 * @param {string} args.title - World title (required)
 * @param {string} [args.background] - Game background and setting
 * @param {string} [args.instructions] - Main instructions for the AI
 * @returns {Promise<{content: Array<{type: string, text: string}>}>} MCP response confirming scaffolding
 */
...
/**
 * Compare two world JSON files and return a structured diff.
 * Returns a detailed comparison report showing changes in root fields, skills array, and all entity arrays.
 * Entity differences are reported as: added entities, removed entities, modified entities with changed fields, and unchanged count.
 * @async
 * @param {Object} args - Function arguments
 * @param {string} args.pathA - Absolute path to the first (base) world JSON file
 * @param {string} args.pathB - Absolute path to the second (comparison) world JSON file
 * @returns {Promise<{content: Array<{type: string, text: string}>}>} MCP response with comparison report showing field changes, added/removed/modified entities, and summary
 */
