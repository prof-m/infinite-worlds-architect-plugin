 ...
 */
export function parseTurns(combinedText, turns) {
  return turns.map(turn => parseSingleTurn(combinedText, turn));
}

/**
 * Parse a single turn's content
 */
function parseSingleTurn(combinedText, turn) {
  const { number: turnNumber, content: turnContent, sourceFile } = turn;

  // Find line range in combined text
  const lineRange = findLineRange(combinedText, turnContent);
 ...
 */
function findLineRange(combinedText, turnContent) {
