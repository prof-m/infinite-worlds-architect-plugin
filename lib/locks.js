 ...
 * On timeout the internal promise chain is unblocked so subsequent waiters
 * are not permanently deadlocked.
 *
...
export async function acquireDraftLock(draftPath) {
    ...
    try {
        ...
        // Timeout fired before we could acquire. Resolve `acquired` so that any
        // subsequent callers who have already chained off our slot are not
        // permanently blocked, then propagate the error.
        release();
    ...
    // Return an idempotent release function so double-calls are harmless.
    let released = false;
    return () => {
        if (released) return;
        released = true;
